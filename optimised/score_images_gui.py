"""
score_images_gui.py (optimised)

Same step-by-step Tkinter viewer as the original score_images_gui.py, with
one change: inference runs on onnxruntime against an exported ONNX model
instead of PyTorch + ultralytics. See ../optimised/README.md for why, and for
the accuracy/speed numbers measured against the original model.

All settings are configured IN THE GUI (no command-line params):
    - Images folder (Browse) - searched recursively, incl. subfolders
    - ONNX model file + its input size (must match how it was exported)
    - Number of random images to sample (default 10)
    - YOLO confidence, display size, random seed
...
SETTINGS_FILE = os.path.join(HERE, 'score_images_gui_settings.json')

DEFAULT_SETTINGS = {
    'images_dir': os.path.join(HERE, '..', 'data', 'darts', 'images', 'small_sample'),
    'weights': os.path.join(HERE, 'models', 'dart_yolo_800_fp16.onnx'),
    'imgsz': 800,
    'conf': 0.5,
    'display_size': 560,
    'num': 10,
...
class ScoreViewer:
    ...
    def _build_ui(self):
        self.root = Tk()
        self.root.title("Dart Sense (optimised) - step-by-step image scorer")
        ...
        self.var_imgsz = StringVar(value=str(self.settings['imgsz']))
        ...
        ttk.Label(s, text="ONNX model:").grid(row=1, column=0, sticky=W, pady=2)
        ...
        ttk.Label(s, text="Model input size:").grid(row=4, column=0, sticky=W, pady=2)
        ttk.Entry(s, textvariable=self.var_imgsz, width=8).grid(row=4, column=1, sticky=W, padx=4)
        ttk.Label(s, text="(must match export - see models/README)").grid(row=4, column=2, columnspan=2, sticky=W)

        btns = ttk.Frame(s)
        btns.grid(row=5, column=0, columnspan=5, pady=(8, 0))
    ...
    def _collect_settings(self):
        ...
        try:
            ...
            messagebox.showerror("Invalid input",
                                 "Num, confidence, model input size and display size must be numbers.")
            return False
        return True
    ...
    def run(self):
        ...
        # (re)load model only if weights/imgsz changed
        weights = self.settings['weights']
        imgsz = self.settings['imgsz']
        if self.model is None or self.loaded_weights != weights or self.loaded_imgsz != imgsz:
            try:
                self.model = OnnxYolo(weights, imgsz=imgsz)
                self.predict = GetScores(weights)
                self.loaded_weights = weights
                self.loaded_imgsz = imgsz
    ...
    def _build_steps(self, path):
        ...
        steps.append(("3. Detections (white=calib, yellow=darts)", det))

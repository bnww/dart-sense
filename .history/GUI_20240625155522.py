...
class GUI:
    ...
    def _game_screen(self):
        ...
        self.canvas.grid(column=1, row=self.scorer.num_players+5, columnspan=9, rowspan=9)
        ...
        ttk.Label(self.mainframe, text="Visit", font=self.heading_font).grid(column=5, row=1, sticky=W)
        ...
        s.configure('Visit_1.TLabel', font=('Helvetica', 12), foreground=self.dart_colours[0])
        ...
        self.visit_labels = []
        for i in range(3):
            s.configure(f'Visit_{i}', font=('Helvetica', 12), foreground=self.dart_colours[i])
            visit_label = ttk.Label(self.mainframe, font=self.current_player_font, style=f'Visit_{i}')
            visit_label.grid(column=5+i, row=2, sticky=W)
            self.visit_labels.append(visit_label)
        ...
        if not self.demo_mode.get():
            ...
            ttk.Label(self.mainframe, text="Display:", underline=0).grid(column=9, row=1, sticky=W)
            ...
            original_button.grid(column=9, row=2, sticky=W)
            transform_button.grid(column=9, row=3, sticky=W)
            live_button.grid(column=9, row=4, sticky=W)
            ...
            add_button.grid(column=8, row=1, sticky=W)
            ...
            commit_button.grid(column=8, row=2, sticky=W)
            ...
            save_button.grid(column=8, row=3, sticky=W)

            self.fps_label = ttk.Label(self.mainframe)
            self.fps_label.grid(column=9, row=self.scorer.num_players+4, sticky=W)
    ...
    def update_scoreboard(self, score, remaining, fps):
        ...
        for i in range(3):
            self.visit_labels[i].configure(text=self.video_processing.darts_in_visit[i])
            self.visit_labels[i].grid_configure(row=self.scorer.current_player+2)

...
class GUI:
    ...
    def _game_screen(self):
        ...
        self.visit_labels = []
        for i in range(3):
            s.configure(f'Visit{i}.TLabel', foreground='black', background=self.dart_colours[i])
            visit_label = ttk.Label(self.mainframe, font=self.current_player_font, borderwidth=2, relief='solid', style=f'Visit{i}.TLabel')
            visit_label.grid(column=5+i, row=2, sticky=N+S + E + W, padx=5, pady=5)

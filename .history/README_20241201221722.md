...
### Score Prediction
In order to use the predicted dart locations from the YOLO model, the coordinates need to be contextualised with reference to some measure of the standardised dart board dimensions. Two approaches were used for this. The first (V1) uses YOLO to predict calibration points
from the image (hereon referred to as the image plane) in order to transform the image onto
the standardised model of the dartboard (the reference plane). The second approach (V2)
uses a Hough transformation to delineate the numbered segments of the board and colour
analysis to differentiate between scoring beds (e.g single, double, treble). The methods for
each approach are detailed separately in the following subsections.

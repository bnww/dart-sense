...
Comparing the F1 scores for a model using trained hyperparameters compared to the default values shows the benefits of tuning. D5 is an especially difficult dataset, taken from a unique video. No data from D5 was used to train the model, so the much improved performance on this data set is indicative of 
